...
the categorical variable, transmission (`am`), i.e. $mpg \sim am$. This returns
the coefficients:
...
[predictions](#appendices) made by this model are just the mileage sample means
for their respective transmission type as listed in the table below:
...
transmission.  Weight reduces mileage `r round(modelMLRsum$coef[2],3)` per 1000
lb increase in weight. While taking longer to do a $^1/_4$ quarter mile improves
mileage by `r round(modelMLRsum$coef[3],3)` per second increase in time.
...
In the [Appendices](#appendices) are residual plots for both models. The first
shows residuals for our [Simple Linear Regression 
