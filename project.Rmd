...
auto-mobile attributes surveyed, the inclusion of just a few namely,
Transmission Type, Weight and $^1/_4$ mile time, provide a better prediction for
mileage than using only the type of transmission.
...
How does mileage compare to transmission? The [Mileage by Transmission 
Type](#appendices) plot would indicate that manual transmissions have better 
mileage. However, this could be due to other confounding factors. We will 
investigate this further in the following sections.

# Simple Linear Regression Model

Consider a simple linear model where we predict the mileage (`mpg`) using just 
...
kable(modelSLRsum$coef, caption = "Model Coefficients")
```
...
amount of linear relationship between mileage and transmission. However, the 
[predictions](#appendices) made by this model are just the mileage sample mean
for their respective transmission type:

| Transmission | Prediction | Mileage Sample Mean |
|--------------|------------|--------------------|
| Automatic | `r round(predict(modelSLR, data.frame(am = 0)), 3)` | `r round(mean(with(mtcars, mtcars[am == 0, "mpg"])), 3)` | 
| Manual | `r round(predict(modelSLR, data.frame(am = 1)), 3)` | `r round(mean(with(mtcars, mtcars[am == 1, "mpg"])), 3)` |

Table: Model prediction vs mileage sample mean

This is further demonstrated by the [plots of residual vs fitted
values](#appendices).
...
As an alternative look at a multi-variable linear model. To assist us we will 
use the [AIC](https://en.wikipedia.org/wiki/Akaike_information_criterion)
...
likelihood (residual standard error) of `r prettyNum(round(modelSLRsum$sigma,3))`
and an AIC of `r round(AIC(modelSLR),3)`.
...
kable(modelMLRsum$coef, caption = "Model Coefficients")
```

We can see that on average manual transmission cars have
`r round(modelMLRsum$coef[4],3)` MPG better mileage than cars with automatic 
transmission. So, how does this model compare to our [Simple Linear Regression 
...
In the [Appendices](#appendices) are two residual plots. The first shows
residuals for our [Simple Linear Regression
Model](#simple-linear-regression-model). The values are clustered around the
sample means. This is an indication of poor model fit. Compare this to the
residuals vs fitted values for the multi-variable regression model. The values
are more evenly spread without any distinct cluster or pattern. This further
indicates a better model fit than the Simple linear Regression Model.

Included in the Appendices is the plot [Multi Linear Regression: Normal 
Q-Q](#appendices). This helps show the fit standardised residuals to the
expected Gaussian distribution.

# Conclusion

The multi-variable linear regression model provides a more resilient predictor
of mileage, although at a small cost of requiring additional regressors, namely
the automobiles weight and the time for a quarter mile. Weight reduces mileage 
`r round(modelMLRsum$coef[2],3)` per 1000 lb increase in weight. While taking
longer to do a $^1/_4$ quarter mile improves mileage by
`r round(modelMLRsum$coef[3],3)` per second increase in time.
...
```{r risvsfitmodelSLR, fig.cap = "Fitted vs Residuals for modelSLR"}
...
```{r risvsfitmodelMLR, fig.cap = "Fitted vs Residuals for modelMLR"}
