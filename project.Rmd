...
predictions made by this model are just the mileage sample means for their
respective transmission type as listed in the table below:
...
This is further demonstrated by [Figure: Fitted vs Residuals for Simple Linear
Regression](#appendices). This plot shows the values are clustered around the
sample means. This is an indication of poor model fit.
...
transmission.  Weight reduces mileage by `r round(modelMLRsum$coef[2],3)` per
1000 lb increase in weight. While taking longer to do a $^1/_4$ quarter mile
improves mileage by `r round(modelMLRsum$coef[3],3)` per second increase in
time.
...
The [Figure: Fitted vs Residuals for Multi Linear Regression](#appendices)
